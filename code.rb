...
twitter_data.each do |hash|
  hash.each do |key,data|
    if data["description"] == nil
      puts "#{hash.keys.first} : N/A"
    else
      puts "#{hash.keys.first} : #{data["description"]}"
    end
  end
end
puts "\n"
...
twitter_data.each do |hash|
  hash.each do |key,data|
    puts "#{hash.keys.first} : #{data["number of followers"]}"
  end
end
puts "\n"
...
twitter_data.each do |hash|
  hash.each do |key,data|
    puts "#{hash.keys.first}'s last tweet was #{data["latest tweet"].length} characters long."
  end
end
puts "\n"
...
twitter_data.each do |hash|
  hash.each do |key,data|
    count = 0
    sum = 0
    data["last twenty tweets"].each do |tweet|
      if count <= 20
        sum = sum + tweet.length
        count += 1
      end
    end
    puts "#{hash.keys.first} used #{sum} characters in their last twenty tweets."
  end
end
puts "\n"
...
twitter_data.each do |hash|
  hash.each do |key,data|
    if data["number of followers"] >= max
      max = data["number of followers"]
      user = key
    end
    if data["number of friends"] >= max_friends
      max_friends = data["number of friends"]
      user_max_friends = key
    end
    if data["number of tweets"] >= max_tweets
      max_tweets = data["number of tweets"]
      user_max_tweets = key
    end
    if data["description"] != nil
      described << key
...
puts "The user with the most followers is #{user}."
#   * Which user has the most friends?
puts "The user with the most friends is #{user_max_friends}."
#   * Which user has the greatest number of tweets?
puts "The user with the greatest number of tweets is #{user_max_tweets}"
puts "\n"
#   * Which users have a `description` listed in `twitter_data`?
puts "These are the users with listed descriptions: "
puts described
puts "\n"
#   * Which users have a `location` listed in `twitter_data`?
puts "These are the users with listed locations: "
puts location
